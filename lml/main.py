...
def lml(model_path: str, data_path: str, video: list, val_save: bool, xgb_save: bool):
    ...
    # Get data from current video, h = 0.75
    if(os.path.exists(data_path)):
        ...
        for i in range(frames_count):
            temp_csv = df.loc[df['filename'] == f'{video}-{"0"*(4 - len(str(i)))}{i}']
            ...
            for in_frame_obj in linked_data:
                if(in_frame_obj._x_c.shape[0] >= i + 1):
                    if(in_frame_obj._x_c[i] != -1):
                        ...
                        width = in_frame_obj._b_w[i]*IMG_WIDTH
                        height = in_frame_obj._b_h[i]*IMG_HEIGHT

                        res.append([i+1, in_frame_obj._name, left, top, width, height, -1, -1, -1, -1])
...
if __name__ == '__main__':
    for video in videos:
        ...
        res_df = lml(model_path=f'./lstm/models/mot/last.pt',
                     data_path=f'./data/mot/MOT20-05.csv',
                     video=video,
                     val_save=True,
                     xgb_save=False)
